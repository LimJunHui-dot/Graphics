...
LightColor CaculateLightColor(int lightIndex, float3 viewNormal, float3 viewPos)
{
    LightColor color = (LightColor) 0.f;
    
    float viewLightDir = (float3) 0.f;
    ...
    {
        // Directional Light
        // view space 기준으로 변환 g_matView을 곱해줘 뷰 행렬 변환을 이룸
        // normalize를 해 단위벡터로 만들어줌
    ...
    {
        // Spot Light
        float viewLightPos = mul(float4(g_light[lightIndex].position.xyz, 1.f), g_matView).xyz;
        viewLightDir = normalize(viewPos, -viewLightPos);
        diffuseRatio = saturate(dot(-viewLightDir, viewNormal));
